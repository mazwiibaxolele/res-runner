...
import { 
  ...
  MapPin, ShoppingCart, User, Phone, ShieldAlert 
} from 'lucide-react';
...
export function RunnerDashboard() {
  ...
  // In our mock database, we look for any order that has status 'assigned' or 'in_progress'
  const activeErrand = orders.find(o => ['assigned', 'in_progress'].includes(o.status));

  // Completed errands by this runner
  const completedErrands = orders.filter(o => o.status === 'delivered');

  // Earnings calculations (Runner receives the service fee + distance fee)
  const totalEarnings = completedErrands.reduce((sum, o) => {
    return sum + o.pricing.service + o.pricing.distance;
  }, 0);
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-brand-bg/50 p-4 border-2 border-white rounded-[20px] space-y-2 text-sm">
                      ...
                      <p className="text-[11px] text-brand-muted mt-1">Approx distance: {activeErrand.distanceKm} km</p>
                    </div>
                  </div>
                  ...
                  {completedErrands.map(order => (
                    <div key={order.id} className="p-3 bg-brand-bg/50 border rounded-xl text-xs space-y-1">
                      <div className="flex justify-between font-bold">
                        ...
                        <span className="text-brand-primary">R{order.pricing.total.toFixed(2)}</span>
                      </div>
